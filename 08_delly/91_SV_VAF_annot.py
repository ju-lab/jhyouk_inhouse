...
#2018-07-16: if read has false SA tag: 'blan' -> it can be reassed by split point.; make tsr_sD for short deletion which is the supporting pairread count is not available; user asr1 and asr2 for shortdeletion; setting shorDel cutoff
...
def find_splitread(chr1, pos1, ter1, chr2, pos2, ter2 ):
	global as1, asp2, as2, asr1, asr2, tsr, tsr_sD
	as1=0; asr1=0;asp2=0; asr2=0; as2=0
	pos1 = int(pos1); pos2 = int(pos2); ter1= int(ter1); ter2=int(ter2)
	if ter1==3: pos1_start=pos1-fors; pos1_end=pos1+bacs
	elif ter1==5: pos1_start=pos1-bacs; pos1_end=pos1+fors
	for read in bam_file.fetch(chr1, pos1-2, pos1+1):   # search +-1 bp
		true_SA='off'; accurate_SC='off'
		if read.is_unmapped == True or read.is_secondary == True or read.is_supplementary == True or read.is_duplicate == True: continue
		if read.has_tag('SA'):
	...
	for read in bam_file.fetch(chr2, pos2-2, pos2+1):   #search +-1bp
		...
		if read.next_reference_name == chr1 and read.next_reference_start+1 >= pos1_start and read.next_reference_start+1 < pos1_end:
			SV_mate = 'on'
		if (true_SA) == 'on' or accurate_SC == 'on':
			as2 += 1
	...
	tsr = as1 + asp2  # use tsr as a split count
	#use as1, asr1 and as2, asr2 for ref read count of short deletion
	...
	if sv_line[0:5]=='#CHR1':
		...
		if pos2 == '.' or svtype == 'INS': 
			...
			if chr1 == chr2 and ter1 == '3' and ter2 == '5' and abs(int(pos2)-int(pos1)) < shortDco:
				shortDstatus = 'on'
				print('hit')
			if tpr==0 or shortDstatus == 'on':
				ref1=asr1; ref2=asr2
			else:
				ref1=r1; ref2=r2
			if shortDstatus == 'on':
				vaf1=str(round(as1*100/float(as1+ref1),2))+'%'
				vaf2=str(round(as2*100/float(as2+ref2),2))+'%'
				tpr='NA'; tsr=str(as1)+';'+str(as2)
			else:
		...
		info_list=[str(ref1),str(ref2),str(tpr),str(tsr),vaf1,vaf2]

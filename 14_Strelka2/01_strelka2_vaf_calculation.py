...
input_file = file(pwd_fn + '/' + input_fn + '/results/variants/somatic.snvs.vcf')
output_file = file(pwd_fn + '/' + input_fn + '/results/variants/somatic.snvs.vaf.vcf',"w")
...
while input_line:
	...
	ref = float(input_info[i].rstrip().split(',')[0])
	alt = float(input_info[j].rstrip().split(',')[0])

sequenza.extract.hg38 <- function(file, gz = TRUE, window = 1e6, overlap = 1, gamma = 80, kmin = 10,
                             ...
                             breaks = NULL, breaks.method = "het", assembly = "hg38", weighted.mean = TRUE,
    ...
    if (nrow(seqz.het) > 0) {
      ...
      if (is.null(breaks.all)){
        if (breaks.method.i == "full") {
          ...
          BAF  <- BAF[!not.cover, ]
          logR <- logR[!not.cover, ]
          logR.wins <- copynumber::winsorize(logR, assembly = assembly, verbose = FALSE)
          allele.seg <- copynumber::aspcf(logR = logR.wins, BAF = BAF, assembly = assembly, baf.thres = c(0, 0.5),

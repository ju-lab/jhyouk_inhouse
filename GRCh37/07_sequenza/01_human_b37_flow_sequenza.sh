...
echo bam2seqz
sequenza-utils bam2seqz -gc /home/users/jhyouk/06_mm10_SNUH_radiation/GRCh37/07_sequenza/gc50base.hg37.wig.gz -n $1/$normalPileup.mpileup -t $1/$sampleName.mpileup -p -o $sampleName.seqz > $sampleName.bam2seqz.out 2>&1
...
echo mtglremove
cat $sampleName.comp.seqz | grep -v 'MT' | grep -v 'GL' | grep -v 'JH' | grep -v 'hs' > $sampleName.comp.seqz.rmGLMTJH 2> $sampleName.mtglremove.out
...
#/home/users/jhyouk/miniconda2/bin/Rscript /home/users/jhyouk/06_mm10_SNUH_radiation/GRCh37/07_sequenza/13_Rscript_sequenza_6args_noploidyassign.R $sampleName.comp.seqz.rmGLMTJH.gz $sampleName-onlypurity $4 $5 $6 $7 &> $sampleName.onlypurity.Rscript.out

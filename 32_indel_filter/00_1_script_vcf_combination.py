...
output_fn = '00_2_run_union_indel' + input_date +  '.sh'
output_file = file(output_fn,'w')

input_line = input_file.readline().strip()
while input_line:
    input_split = input_line.split(' ')
    input_ID = input_split[2]
    output_file.write('python 00_vcf_combination_by_Youk_indel.py %s %s 2 /home/users/jhyouk/06_mm10_SNUH_radiation/14_Strelka2/%s/results/variants/somatic.indels.vcf.gz /home/users/jhyouk/06_mm10_SNUH_radiation/09_varscan/%s.varscan.indel.somatic.vcf &> %s.union.out\n' % (input_ID,input_species,input_ID,input_ID,input_ID))
    input_line = input_file.readline().strip()
